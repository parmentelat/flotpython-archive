...
Pour les anciennes classes, le mécanisme d'héritage multiple est plus
...
Après quelques errements, décrits dans la première référence (voir ci-dessous la
dernière section, "Pour en savoir plus"), l'algorithme qui a été choisi pour
déterminer l'ordre des superclasses, qui est en service depuis la version 2.3,
est connu sous le nom de **linéarisation C3**. Cet algorithme n'est pas propre à
python, comme vous pourrez le lire dans les références citées dans la dernière
section.

Nous ne décrirons pas ici l'algorithme lui-même dans le détail; par contre nous
allons&nbsp;:
 * dans un premier temps essayer de résumer **les raisons** qui ont guidé ce
choix, en résumant les bonnes propriétés que l'on attend, ainsi que les
**limitations** qui en découlent;
 * et voir l'ordre obtenu sur quelques exemples concrets de hiérarchies de
classes.
...
De la même façon, lorsqu'une classe A hérite d'une classe B, on s'attend à ce
que les méthodes définies sur B, qui sont en principe plus spécifiques, soient
utilisées de préférence à celles définies sur A.
...
hériter de plus de deux classes, tout ce qui est demandé est que C et D
apparaissent, avec en premier dans la liste des classes mères.
...
pas trouver un tel ordre.

Nous en verrons un exemple vers la fin de ce document; comme vous le verrez on
peut exhiber de tels exemples qui restent relativement simples. Cependant dans
la pratique il est relativement rare de tomber sur de tels cas pathologiques; et
lorsque cela se produit c'est en général le signe d'erreurs de conception plus
profondes.
...
On se donne la hiérarchie suivante.
    ...
    class LeftTop (object):
        def attribut(self): return "attribut(LeftTop)"
    class LeftMiddle(LeftTop): pass
    class Left(LeftMiddle): pass
    class Middle (object): pass
    class Right (object):
        def attribut(self): return "attribut(Right)"
...
`Right` `object` par `LeftMiddle`; de quelle classe d'après vous est-ce que
`Class` hérite `attribut` dans cette configuration ?
...
section, "Pour en savoir plus"). Cet exemple utilise des classes *new-style*.

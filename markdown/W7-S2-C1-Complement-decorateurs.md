 ...
 * et si on ne trouve pas les arguments dans le cache, on reçoit l'exception
...
;:
    ...
    def caching(a_decorer):
        ...
        # propriétés de a_decorer wrt __doc__ et __name__
...
n'existe qu'en un seul exemplaire; tous les appels au constructeur retournent
une référence vers le même objet.
...
comprend quand on sait qu'elle est appelée **pour** créer l'objet; on ne peut
donc pas lui passer en paramètre l'objet en cours de création, puisqu'il n'est
pas encore là.
...
C'est donc que l'instance de la classe singleton est considéré comme une classe;
cette instance doit être un callable, qui est utilisé lorsqu'on crée une
instance de `Spam` en faisant

    obj1 = Spam()

Notre décorateur, pour l'essentiel, fait ceci&nbsp;:
 * dans le constructeur `singleton`, l'objet décoré mémorise la classe à décorer
(ici `Spam`) dans son attribut `classe`;
 ...
 * `ingleton_new` est *installée* dans la classe décorée, dans l'attribut de nom
`__new__`;
 * et pour être complet on met à jour `__name__` et `__doc__` dans la classe
décorée à partir de la classe originale.

### Toutes les combinaisons (fonction, classe) x (fonction, classe)

Nous avons choisi d'implémenter `singleton` comme une classe; vous [trouverez
ici une version implémentée comme un
...
Vous avez toutes les combinaisons de décorateurs: de classe ou de fonction,
implémenté comme une classe ou comme une fonction.
...
Ce paramètre, appelons_le `cache_timeout`, on veut pouvoir le préciser pour
chaque fonction; par exemple on voudrait écrire quelque chose comme
...
qui suit, est le suivant&nbsp;:
 * `caching_expire` est ce qu'on appelle une *factory* à décorateurs,
 * c'est-à-dire que le langage va d'abord appeler `caching_expire(600)` et que
ceci doit retourner un décorateur,
 * qui lui même doit se comporter commes les décorateurs sans argument que l'on
a vus jusqu'ici.
...
Signalons enfin que l'on peut aussi "chaîner les décorateurs"; imaginons par
exemple qu'on dispose d'un décorateur `add_field` qui ajoute dans un classe un
*getter* et un *setter* basés sur un nom d'attribut.
...
problème est bien souvent la création d'objets supplémentaires: on n'appelle
plus la fonction de départ mais un wrapper autour de la fonction de départ.
...
- encore nouvelle - version de fibonacci, le code obtenu ne converge pas; en
fait les technique que nous avons utilisées dans les deux cas ne sont pas
compatibles entre elles.

De manière plus générale, il se trouve des gens pour trouver des défauts à ce
système de décorateurs; je vous renvoie notamment à [ce
...
chose qui peut être très très utile, mais dont il ne faut pas abuser. En
particulier de notre point de vue, la possibilité de combiner les décorateurs,
si elle existe bien dans le langage d'un point de vue syntaxique, est dans la
pratique à utiliser avec la plus extrême prudence.
